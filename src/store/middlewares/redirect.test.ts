...
describe('Redirect middleware', () => {
  ...
  it('should not redirect to "/films" with empty action', () => {
    const emptyAction = { type: '', payload: AppRoute.Films };
    store.dispatch(emptyAction);
    expect(browserHistory.location.pathname).not.toBe(AppRoute.Films);
  });
});
